...
void DAC_Init(uint16_t data){
  SYSCTL_RCGCSSI_R |= 0x04;       // activate SSI0
  SYSCTL_RCGCGPIO_R |= 0x02;      // activate port A
  while((SYSCTL_PRGPIO_R&0x02) == 0){};// ready?
  GPIO_PORTB_AFSEL_R |= 0xB0;     // enable alt funct on PA2,3,5
  GPIO_PORTB_DEN_R |= 0xB0;       // configure PA2,3,5 as SSI
...
void DAC_InitRaw(void){
  SYSCTL_RCGCSSI_R |= 0x0;       // activate SSI0
